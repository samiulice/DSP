...
for n = 1:DFT_points
    m = n-1;
    x(i) = sin(2*pi*1000*m*Ts)+0.5*sin(2*pi*2000*m*Ts+(3*pi/4));
    i = i + 1;
end
t = 1:DFT_points;
...
X_mag = zeros(N,1);    Xe_ang_rad = zeros(N,1);  Xe_ang_deg = zeros(N,1);
for m = 1:DFT_points
    X_1 = zeros(N,1);   
    m_e = m - 1;
    for ii = 1:N
    ...
    X_mag(m,1) = sqrt(Xe_real(m,1).^2 + Xe_imag(m,1).^2);
