...
for n = 1:DFT_points
    m = n-1;
    x1(i) = sin(2*pi*1000*m*Ts);
    i = i + 1;
end
t = 1:DFT_points;
% Plot first N discrete values of continuous signal x :
   figure(1);   plot(t,x1,'b--o');   grid on;
...
X1_mag = zeros(N,1);    Xe_ang_rad = zeros(N,1);  Xe_ang_deg = zeros(N,1);
for m = 1:DFT_points
    X_1 = zeros(N,1);   
    m_e = m - 1;
    for ii = 1:N
        n = ii-1;   
        X_p1 = [];   
        X_p1 = x1(1,ii)*exp((-j*2*pi*n*m_e)/N);                    
        X_1(ii,1) = X_p1;  
    end
    X_exp(m,1) = sum(X_1(:,1));
    ...
    X1_mag(m,1) = sqrt(Xe_real(m,1).^2 + Xe_imag(m,1).^2);
